...
library(gammadnamix); #sessionInfo()

#model:
nC <- 2 #number of contributors
threshT = 150 #detection threshold (default is 50)
#xi <- NULL #specify that stutter ratio (xi) is an unknown parameter (this is default)
#pXi <- function(x) return(1) #prior to the stutter ratio as flat (this is default)
#prC <- 0 #assume no Drop-in (this is default)
...
set.seed(1) 
gendata <- genDataset(nC,popFreq,mu=mu,sigma=sigma,mx=mx,threshT=threshT,nrep=nrep,stutt=xi0)
...
hpH[nC] <- 1 #condition ref2 to be placed as contributor 1 in model
...
#given hpDmle-object: Do deconvolution
dlisthp <- deconvolve(mlefit=hpmle,alpha=0.99,maxlist=1000) #time depends on discrimination of sample
#barplot(dlisthp$pG)
#head(dlisthp$table1)

LRmlei <- exp(hpmlelogliki - hdmlelogliki) #MLE LR for each locus
LRmle <- exp(hpmle$fit$loglik - hdmle$fit$loglik) #MLE LR = prod(LRmlei)
LRlap <- exp(hpmle$fit$logmargL - hdmle$fit$logmargL) #Laplace LR (should be about half value of LRmle)

###################################
#MCMC Metropolis Hasting sampling:#
##Used to explore parameter space##
###################################
M <- 1e3 #number of posterior-samples
set.seed(1)
delta <- 8 #try find a good behaved delta such that acceptance rate is around 0.25 and acf low
hdmcmc <- contLikMCMC(mlefit=hdmle,M=M,delta=delta)
print(hdmcmc$accrat) #acceptance rate of sampler
validMCMC(mcmcobj=hdmcmc,acf=FALSE)

M <- 1e3 #number of posterior-samples
set.seed(1)
delta <- 8  #try find a good behaved delta such that acceptance rate is around 0.25 and acf low
hpmcmc <- contLikMCMC(mlefit=hpmle,M=M,delta=delta)
print(hpmcmc$accrat)#acceptance rate of sampler
validMCMC(mcmcobj=hpmcmc,acf=FALSE)
...
hdL <- hpL <- c(rep(0,nC-1),400,0,0)
hdU <- hpU <- c(rep(1,nC-1),800,0.3,0.4) #assume upper limit of stutter ratio as 0.4
...
LRint <- hpint$margL/hdint$margL
LRdev <- c(hpint$dev[1]/hdint$dev[2],hpint$dev[2]/hdint$dev[1]) #error-interval of LR
