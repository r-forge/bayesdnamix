...
validMCMC <- function(X,trace=TRUE,acf=TRUE,mlefit=NULL,isPhi=FALSE) {
 ...
 for(i in 1:p) {
  ...
  dens <- density(X[,i])
  plot(dens,ty="l",main=txt)
  if(!is.null(mlefit)) {
   if(isPhi) lines(dens$x,dnorm(dens$x,mlefit$theta0[i],sqrt(mlefit$Sigma0[i,i])),col=2,lty=2)
   if(!isPhi) lines(dens$x,dnorm(dens$x,mlefit$theta1[i],sqrt(mlefit$Sigma1[i,i])),col=2,lty=2)
  }
  if(trace) plot(X[,i],ty="l",ylab=txt)
